...
$rubrique='settings';
$page_name='List of payment method';
...
if(isset($_GET['typ_search']) AND isset($_GET['val_search']))
{
  ...
  $val_search = $_GET['val_search'];
   ...
   if($typ_search == 'method' AND !empty($val_search))
   {
     $sql_search = ' AND method = "'.$conn->escape_string($val_search).'" ';
   }
   else
   {
     $sql_search = '';
...
else
{
  $nbrepge = 10;
   ...
   if(isset($_GET['del']))
   {
	  $id_delete = $conn->escape_string($_GET['del']);
	   ...
	   if($id_delete == '')
		...
		{	
			...
			$req_SUP = "SELECT * FROM method_payment WHERE id_method='$id_delete'";
	...
	$req_DES = $conn->query("UPDATE method_payment SET status = '0' WHERE id_method='$id_delete'"); 		   
  ...
  $sql = 'SELECT *,DATE_FORMAT(created, "%d/%m/%Y %Hh%imin%ss") AS date1 
			FROM method_payment WHERE status = 1 '.$sql_search; 
							...
							<div class="module-head">
								...
								<div class="module-option clearfix">
                                    ...
                                    <div class="pull-right">
                                        <a href="payment.php" class="btn btn-primary">Add Payment Method</a>
                                    </div>
                                </div>		
	    ...
	    <tr>
		  ...
		  <td> 
		     <select name="typ_search" tabindex="1" data-placeholder="Selectionner ici.." class="span2">
			    <option value="method" <?php echo ($typ_search == "method")?'selected="selected" ':''; ?> > payment method </option>
			 </select>
			</td> 
			<td>
...
<?php
 ...
 $sql .= " ORDER BY id_method DESC LIMIT $premierProduitAafficher,$nombreDeProduitParPage ";								
...
<?php echo $count ?> payment method(s) <br />
...
<?php 
			...
			?>	
   <tr>
									  <td><?php echo stripslashes($solution['method']); ?></td>
									  <td><?php echo stripslashes($solution['date1']); ?></td>
									  <td>


<a href="<?php echo $page_retour; ?>?del=<?php echo $solution['id_method']; ?>" onclick="return(confirm('Do you really want to delete it?'));" title="delete">
<img src="<?php echo $path ?>images/delete_icon.png" />
</a>

&nbsp;

<a href="payment.php?id=<?php echo $solution['id_method']; ?>" title="Edit">
